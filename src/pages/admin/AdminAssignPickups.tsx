...
export default function AdminAssignPickups(): JSX.Element {
  ...
  async function loadCollectors() {
    ...
    try {
      // IMPORTANT: Start with endpoints that typically exist in your backend.
      // This avoids the /users?role=collector 404.
      // 1) /admin/collectors
      try {
        const res = await api.get("/admin/collectors");
        const raw = res?.data;
        const list: CollectorRaw[] = Array.isArray(raw) ? raw : raw?.data ?? raw?.collectors ?? [];
        if (list.length > 0) {
          setCollectors(normalize(list));
          return;
        }
      } catch {}
      ...
      // 3) /users?role=collector  (only if your backend supports it)
      try {
      ...
      // 4) fallback: filter /users (only if /users exists)
      try {
        const res = await api.get("/users", { params: { limit: 500 } });
        const list: any[] = Array.isArray(res.data) ? res.data : res.data?.data ?? [];
        const onlyCollectors = list.filter((u: any) => {
          const role = (u?.role ?? "").toLowerCase();
          return role === "collector" || (u?.tags ?? []).includes("collector");
        });
        if (onlyCollectors.length > 0) {
          setCollectors(normalize(onlyCollectors));
          return;
        }
      } catch {}

      setCollectorsError("No collectors found. Check backend endpoints and ensure collector accounts exist.");
    } catch (err: any) {
  ...
  async function submitAssign() {
    ...
    try {
      // If your backend expects /admin/waste/:id/assign, change this endpoint accordingly.
  ...
  const wasteTypes = useMemo(
    () => [...new Set(items.map((i) => i.wasteType).filter(Boolean))] as string[],
    [items]
  );
  ...
  return (
    <div className="aap-root">
      ...
      <main className="aap-main">
        ...
        {!loading && !error && filtered.length === 0 && <div className="aap-empty">No unassigned pickups found.</div>}
              ...
              <div key={p._id} className="aap-card">
                <div className="aap-card__info">
                  ...
                  {(p.location ?? p.user?.address) && (
                    <div className="aap-card__location">📍 {p.location ?? p.user?.address}</div>
                  )}
                </div>

                <div className="aap-card__actions">
                  <button className="aap-btn aap-btn--sm" onClick={() => navigate(`/track/${p._id}`)}>
                    Track ↗
                  </button>
                  <button className="aap-btn aap-btn--primary aap-btn--sm" onClick={() => openAssignModal(p._id)}>
                    Assign
                  </button>
                </div>
              </div>
          ...
          >
            <div className="aap-modal" role="dialog" aria-modal="true" aria-labelledby="aap-modal-title">
              <div className="aap-modal__head">
                <span className="aap-modal__title" id="aap-modal-title">
                  Assign Pickup
                </span>
                <button className="aap-modal__close" onClick={closeAssignModal} aria-label="Close">
                  ✕
                </button>
              </div>
              ...
              <div className="aap-modal__body">
                {loadingCollectors ? (
                  ...
                  <>
                    ...
                    <button className="aap-btn" onClick={loadCollectors}>
                      ↻ Retry
                    </button>
                  </>
                ) : collectors.length === 0 ? (
              ...
              {!loadingCollectors && !collectorsError && collectors.length > 0 && (
                <div className="aap-modal__foot">
                  <button className="aap-btn" onClick={closeAssignModal} disabled={assigning}>
                    Cancel
                  </button>
                  <button className="aap-btn aap-btn--primary" onClick={submitAssign} disabled={!selectedCollector || assigning}>
        ...
        {toast && (
          <div className="aap-toast" role="status" aria-live="polite">
            <div className={`aap-toast__inner aap-toast__inner--${toast.kind ?? "success"}`}>{toast.text}</div>
          </div>
        )}
      </main>
    </div>
